...
function NewEventProductDialog({ onAdd, eventId }: { onAdd: (product: EventProduct) => void, eventId: string }) {
  ...
  const handleCreateProduct = async () => {
    try {
      const uploadedImageUrls = await Promise.all(images.map((image) => ImageAPI.uploadImage(image)))
      const payload = {
        eventId: eventId,
        name,
        description,
      ...
      if (response.isSuccess) {
        const newProduct: EventProduct = {
          ...
          price,
          quantityInStock,
          productImages: uploadedImageUrls.map(url => ({ id: '', imageUrl: url, name: '' })),
      ...
      <DialogContent className="sm:max-w-[425px] bg-background/50 backdrop-blur-xl">
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            ...
            <Input type="number" id="quantityInStock" placeholder="0" value={quantityInStock} onChange={(e) => setQuantityInStock(Number(e.target.value))} />
          </div>
          <div className="grid gap-2">
...
function ProductItem({ name, price, quantityInStock, productImages }: EventProduct) {
  return (
    <div className="bg-background/50 backdrop-blur-xl rounded-lg p-3 flex justify-between items-center">
      ...
      <div className="flex items-center">
        ...
        <Button variant="ghost" size="sm" className="text-gray-400 hover:text-white p-0 ml-4">
