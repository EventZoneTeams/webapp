...
import { EventPost as EventPostAPI } from "@/lib/api/event-post";
import { AddPostDialog } from "@/app/(root)/dashboard/@organizer/events/[slug]/component/add-post-dialog";
...
export default function EventPost({ event }: { event: Event }) {
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        const response = await EventPostAPI.getPostsByEventId(event.id);
  ...
  return (
    <div className="mx-auto mt-4 max-w-4xl">
      <div className="mb-6 flex items-center justify-between">
        ...
        <AddPostDialog eventId={event.id} />
      </div>

      {posts.length === 0 ? (
        <Card className="border-none bg-primary/5 p-6 text-center">
          <p className="text-lg font-semibold">No Posts yet</p>
          <p className="mt-2 text-muted-foreground">
            Be the first to create a post for this event!
          </p>
        </Card>
      ) : (
        <>
